...
const ManageSingle = (props) => {
  ...
  const handleDelete = () => {
    ...
    if (proceed) {
      fetch(`https://travely-server.onrender.com/visitors/${_id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
    ...
    <div>
      <div className="p-6 shadow-xl h-full">
        <div className="text-right">
          <button className="py-1 px-3 bg-red-200 font-bold rounded text-red-700 my-2" onClick={handleDelete} title="Cancel Booking">
            X
          </button>
        </div>
        <div>
        ...
        <div>
          ...
          {status === "Pending" ? <p className="text-red-500 font-bold">Booking {status}...</p> : <p className="text-green-700 font-bold text-lg">Booking {status}!</p>}
          {status === "Pending" && (
            <button className="px-3 py-2 rounded font-medium bg-blue-900 text-white mt-2" onClick={handleApproved}>
              Approved Booking
            </button>
          )}
        </div>
